...
set -euo pipefail
cd "$(dirname "$0")/.."
FIX=Tests/AIStatusBarTests/Fixtures; mkdir -p "$FIX"
...
for f in ("Tests/AIStatusBarTests/Fixtures/claude-usage.json",
          "Tests/AIStatusBarTests/Fixtures/codex-usage.json"):
    d = json.load(open(f))
